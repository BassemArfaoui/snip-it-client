...
export class ProfileCardComponent {
  ...
  @Input() userId?: number;
  @Output() profileLoaded = new EventEmitter<ProfileSummary>();

  @Output() followChange = new EventEmitter<{ isFollowing: boolean; followers: number }>();
  @Output() edit = new EventEmitter<void>();

  internalFollowLoading = false;
  ...
  loadProfile(): void {
    ...
    this.profileService.getProfile(this.userId).subscribe({
      ...
      error: (err) => {
        ...
        this.error = status === 404 ? 'User not found' : err?.error?.message || 'Failed to load profile';
      }
    });
  }
  ...
  handleFollow(): void {
    ...
    this.internalFollowLoading = true;
    ...
    action.subscribe({
      next: (res) => {
        ...
        this.internalFollowLoading = false;
